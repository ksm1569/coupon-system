...
(
    ...
    `discount_amount`       INT         NOT NULL COMMNET '할인 금액',
    `min_available_amount`  INT         NOT NULL COMMENT '최소 사용 금액',
    `date_issue_start`      datetime(6) NOT NULL COMMNET '발급 시작 일시',
    `date_issue_end`        datetime(6) NOT NULL COMMNET '발급 종료 일시',
    `date_created`          datetime(6) NOT NULL COMMNET '생성 일시',
    `date_updated`          datetime(6) NOT NULL COMMNET '수정 일시',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
...
(
    ...
    `date_issued`   datetime(6) NOT NULL COMMNET '발급 일시',
